...
# --- Configuration and Mappings (keep these as is) ---
load_dotenv()
...
# --- Utility Functions (modified to accept file path and browser context) ---
...
def login(context, page):
    """Handles login and saves the session state."""
...
def is_logged_in(page):
    """Checks if the user is already logged in by looking for a specific element."""
    page.goto(BASE_URL)
    try:
...
def fill_supplier_form(page, record):
    """Fills out the supplier form on the webpage."""
...
def fill_manifest_fields(page, manifest_data):
    """Fills out the manifest setup page."""
    manifest_fields = {
...
async def upload_file(files: UploadFile = File(...)):
    ...
    """
    if not files.filename.endswith(('.csv', '.xls', '.xlsx')):
    ...
    try:
        ...
        with NamedTemporaryFile(delete=False, suffix=os.path.splitext(files.filename)[1]) as temp_file:
            shutil.copyfileobj(files.file, temp_file)
            temp_file_path = temp_file.name

        records = parse_file_to_json_list(temp_file_path)

        # Main Playwright automation loop
        with sync_playwright() as p:
            ...
            for record in records:
                try:
                    fill_supplier_form(page, record)
                    time.sleep(1)
                    ...
                    try:
                        ...
                        if passenger_list_link.is_visible():
